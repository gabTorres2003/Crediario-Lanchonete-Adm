const app = {
  ...
  updateClient: async () => {
    ...
    const { error } = await window._supabaseClient
      .from('clientes')
      .update({ nome: nome, telefone: cleanPhone })
      .eq('id', id)
  ...
  addToRecents: (client) => {
    let recents = JSON.parse(localStorage.getItem('lanchonete_recents') || '[]')
  ...
  paySelectedItems: () => {
    ...
    if (selectedCheckboxes.length === 0) {
      return ui.alert(
        'Atenção',
        'Selecione pelo menos um item para pagar.',
        'warning'
      )
    }
  ...
  savePayment: async () => {
    ...
    if (!amount || amount <= 0)
      return await ui.alert('Atenção', 'Valor inválido', 'warning')
  ...
  generatePDF: (type = 'completo', start = null, end = null) => {
    ...
    if (type === 'selecionados') {
      ...
      if (selectedIds.length === 0)
        return ui.alert('Atenção', 'Nenhum item selecionado!', 'warning')
      ...
      if (!start || !end)
        return ui.alert('Atenção', 'Selecione as datas.', 'warning')
    ...
    if (type !== 'pendentes') {
      doc.text(
        `Total Pedidos: ${totalOrder.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL',
        })}`,
        14,
      ...
      doc.text(
        `Total Pagos: ${totalPay.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL',
        })}`,
        14,
      ...
      doc.setFontSize(14)
      doc.text(
        `Saldo do Período: ${displayBalance.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL',
        })}`,
        14,
      ...
      doc.setFontSize(14)
      doc.text(
        `Total Pendente: ${displayBalance.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL',
        })}`,
        14,
